...
C
     ...
     &                         FLAKUZF,FSNKUZF,NROW,NCOL,NLAY,BUFF
      ...
      IMPLICIT     NONE
      INTEGER      MTWEL,MTDRN,MTSDR,MTRCH,MTEVT,MTRIV,MTGHB,MTCHD,
...
C
C--ALLOCATE
      ALLOCATE(NPERFL,ISS,IVER,FWEL,FDRN,FSDR,FRCH,FEVT,FRIV,FGHB,
...
C--READ HEADER OF FLOW-TRANSPORT LINK FILE
        IF(IFTLFMT.EQ.0) THEN
          READ(INFTL,ERR=100,IOSTAT=IERR) VERSION,MTWEL,MTDRN,MTSDR,
     &                                    MTRCH,
  ...
  100   IF((VERSION(1:4).NE.'MT3D'.AND.VERSION(1:4).NE.'MTGS').OR.
     &     IERR.NE.0) THEN
        ...
        ELSEIF(VERSION(1:11).EQ.'MT3D4.00.00') THEN
          REWIND(INFTL)
          IF(IFTLFMT.EQ.0) THEN
            READ(INFTL) VERSION,MTWEL,MTDRN,MTSDR,MTRCH,MTEVT,MTRIV,
     &                  MTGHB, 
...
C--DETERMINE IF THE SSM PACKAGE IS REQUIRED
  200   IF(iUnitTRNOP(3).EQ.0) THEN
          IF(FWEL.OR.FDRN.OR.FSDR.OR.FRCH.OR.FEVT.OR.FRIV.OR.FGHB.OR.
...
C
     ...
     &                         FWEL,FDRN,FSDR,FRCH,FEVT,FRIV,FGHB,
...
C--READ DRAIN FLOW TERM (L**3/T) IF DRAIN OPTION USED IN FLOW MODEL.
     ...
     &              BUFF,IQ,MXSS,NTSS,NSS,SS,ICBUND,FPRT)
      ENDIF
C
C--READ SDR FLOW TERM (L**3/T) IF SDR OPTION USED IN FLOW MODEL.
      IF(FSDR) THEN
        TEXT='SDR'
        IQ=70
        CALL READPS(INUF,IOUT,NCOL,NROW,NLAY,KSTP,KPER,TEXT,
