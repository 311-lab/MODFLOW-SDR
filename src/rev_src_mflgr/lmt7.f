...
C--USE FILE SPECIFICATION of MODFLOW-2005
      ...
      DATA          INLMT,MTBCF,MTLPF,MTHUF,MTWEL,MTDRN,MTSDR,MTRCH,
     &              MTEVT,
...
C--WRITE A HEADER TO MODFLOW-MT3DMS LINK FILE
     ...
     &     MTWEL,MTDRN,MTSDR,MTRCH,MTEVT,MTRIV,MTGHB,MTCHD,MTISS,MTNPER,
...
C--COLLECT AND SAVE ALL RELEVANT FLOW MODEL INFORMATION
     ...
     &   CALL LMT7DRN7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
        IF(IUNIT(70) .GT.0) 
     &   CALL LMT7SDR7(ILMTFMT,IUMT3D,KKSTP,KKPER,IGRID)
        IF(IUNIT(8) .GT.0) 
...
C
C
            SUBROUTINE LMT7SDR7(ILMTFMT,IUMT3D,KSTP,KPER,IGRID)
C ********************************************************************
C SAVE SDR CELL LOCATIONS AND VOLUMETRIC FLOW RATES FOR USE BY MT3D.
C ********************************************************************
C Modified by Yang
C last modified: 12-01-2019
C
      USE GLOBAL,      ONLY:NCOL,NROW,NLAY,IBOUND,HNEW,BOTM
      USE GWFLPFMODULE, ONLY:HK
      USE GWFSDRMODULE,ONLY:NSDR,SDRR
      CHARACTER*16 TEXT
      DOUBLE PRECISION QQ,DHOOG,LHOOG,RHOOG,SHOOG,QHOOG,AHOOG,
     &                 DDHOOG,HJ,HS,QQHOOG
C    
C--SET POINTERS FOR THE CURRENT GRID
c swm: already set in GWF2SDR7BD      CALL SGWF2SDR7PNT(IGRID)
C      
      TEXT='SDR'
      ZERO=0.
C
C--WRITE AN IDENTIFYING HEADER
      IF(ILMTFMT.EQ.0) THEN
        WRITE(IUMT3D) KPER,KSTP,NCOL,NROW,NLAY,TEXT,NSDR/2
      ELSEIF(ILMTFMT.EQ.1) THEN
        WRITE(IUMT3D,*) KPER,KSTP,NCOL,NROW,NLAY
        WRITE(IUMT3D,*) TEXT,NSDR/2
      ENDIF
C
C--IF THERE ARE NO SDR THEN SKIP
      IF(NSDR.LE.0) GO TO 9999
C
C--FOR EACH DRAIN ACCUMULATE DRAIN FLOW
      DO L=1,NSDR/2
C
